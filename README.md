...
The repository includes `render.yaml` for the API and `client/vercel.json` for Vite's
single-page-app routing.

### 1. Deploy the API on Render

1. Push this repository to GitHub, then in Render choose **New → Blueprint** and select
   the repository. Render will read `render.yaml` and create the `lifelink-api` web service.
2. Set these values in the Render service environment:
   ...
   `JWT_SECRET` is generated by the Blueprint. Keep it stable after users have signed in:
   changing it invalidates existing login tokens.
3. In MongoDB Atlas, allow network access from Render (for a simple first deployment,
   `0.0.0.0/0`; restrict it later where possible). Copy the API URL Render gives you, such as
   `https://lifelink-api.onrender.com`.

### 2. Deploy the client on Vercel

1. Import the same GitHub repository in Vercel and set **Root Directory** to `client`.
   Vercel detects Vite; the build command is `npm run build` and the output directory is `dist`.
2. Add this Vercel environment variable for the **Production** environment:

   ```ini
   VITE_API_URL=https://lifelink-api.onrender.com
   ```

   Use your actual Render URL and do not include a trailing slash.
3. Deploy, then copy the final Vercel URL into Render's `CLIENT_URL` and redeploy the Render
   service if you set it after the first deployment.

The browser connects to `VITE_API_URL/api` for HTTP and `VITE_API_URL` for Socket.IO. Both
the Express CORS policy and Socket.IO CORS policy use `CLIENT_URL`, so the two URLs must match
